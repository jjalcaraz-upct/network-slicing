...
Source code of a network slicing environment where time-frequency resources (radio frame resource blocks, RBs) must be allocated among several network slices at each decision stage. The environtment implements the OpenAI Gym https://github.com/openai/gym interface and interacts with Stable-Baselines RL agents https://github.com/hill-a/stable-baselines and Keras-RL agents https://github.com/keras-rl/keras-rl. A model-based RL control algorithm (KBRL) is included. The objective of the control agent is to allocate the resources efficiently while statisfying the service level agreements (SLAs) of the network slices. At each decision stage, the control agent observes a set of variables describing the state of the system, and an indicator of the SLA fulfillment for each slice. Based on these observations, the agent selects a resource allocation with which the system will operate until the next decision stage.
